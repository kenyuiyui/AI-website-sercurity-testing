...
const FINDING_GUIDE = {
  ...
  insecure_python_exec: {
    plain: 'Python 的 exec() 會把傳進去的文字當成 Python 程式碼直接執行，而這裡傳進去的文字是用 % 格式化、f-string 或 .format() 把變數組進去的。如果組進去的變數來自使用者輸入（例如網址參數、表單內容），使用者就能在裡面夾帶任意 Python 程式碼，讓伺服器照著執行，這叫程式碼注入（Code Injection），風險等級跟 eval() 相同，嚴重時可能取得整台伺服器的控制權。',
    handoff:
      '我的 Python 程式碼裡使用了 exec()，而且傳入的程式碼字串是用 % 格式化、f-string 或 .format() 把變數組進去的，這樣的寫法如果變數來自使用者輸入會有程式碼注入的風險。請幫我：\n' +
      '1. 先確認組進 exec() 的變數是否可能來自使用者輸入或外部資料，如果是，這是需要優先處理的高風險問題。\n' +
      '2. 找出這段程式碼原本想達成的目的，改用不需要執行任意程式碼的做法（例如要動態載入模組就用 importlib.import_module 並搭配白名單，要動態取屬性就用 getattr 並搭配白名單），而不是組字串再 exec()。\n' +
      '3. 檢查專案裡還有沒有其他地方使用 exec() 或 eval()。\n' +
      '完成後請告訴我：原本 exec() 的用途是什麼，以及你改用了什麼替代方案。'
  },
...
const CONTEXT_NOTES = {
  ...
  'site-csp': '這個專案已經有一處「整站生效」的安全防線設定（寫在伺服器設定、框架設定檔，或建置時統一注入的腳本裡），每一頁都會套用，所以這些網頁的原始檔不必各自再寫一份，列為參考。'
};
