...
案例不可以讓人認出是從誰的專案抓來的：不寫 repo 名稱、帳號、網址、獨特的資料夾或檔名，
變數與欄位改成通用名稱。唯一的例外是已經正式公開揭露、有 CVE 或廠商公告的資安事件，
那種可以在 `===SOURCE===` 具名引用來源。

去識別化必然要改寫程式碼，而改寫會帶進改寫者的預期，所以**改寫過的案例一律放
`reference_cases/`，不進 `cases/` 的統計**（理由見該資料夾的 README）。
