...
- **驗證**：本專案自己 38 檔 → 使用中 19、另一個網頁 2、工具／設定 11、測試／範例 6，無法判斷 0；Propose_Minutes 104 檔 → 疑似沒用到 14（`tmp_home.html` 改歸「另一個網頁」）；LIFF 專案 3 檔 → `sw.js` 靠字串路徑認出有在使用。新增 10 項回歸檢查。

## 2026-09 第六輪：單檔式網站整份被略過、結果畫面太雜

起因：使用者拿兩個專案回測。一是本專案自己，結果顯示 18 項，使用者以為是新的 bug（實際上 18 項全是「參考」，都在 `eval/`、`scripts/` 的測試檔，需要處理與請你確認都是 0）；二是 `voice-to-text-line-bot-LIFF`，一個所有程式碼都寫在 `index.html` 的單檔式網站，結果只檢查到 2 個檔案、回報「沒問題」。

- **檔案大小上限 300KB → 2MB**：LIFF 專案的 `index.html` 有 307KB，剛好超過上限被整份略過，等於網站的程式碼一行都沒看，報告卻說沒問題。實測 435KB 的檔案掃描只花 135ms，300KB 訂得太保守。
